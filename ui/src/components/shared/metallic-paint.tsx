...
export default function MetallicPaint({
  ...
  useEffect(() => {
    ...
    gl.uniform1f(u.u_seed ?? 0, seed);
    gl.uniform1f(u.u_scale ?? 0, scale);
    gl.uniform1f(u.u_refract ?? 0, refraction);
    gl.uniform1f(u.u_blur ?? 0, blur);
    gl.uniform1f(u.u_liquid ?? 0, liquid);
    gl.uniform1f(u.u_bright ?? 0, brightness);
    gl.uniform1f(u.u_contrast ?? 0, contrast);
    gl.uniform1f(u.u_angle ?? 0, angle);
    gl.uniform1f(u.u_fresnel ?? 0, fresnel);

    const light = hexToRgb(lightColor);
    const dark = hexToRgb(darkColor);
    const tint = hexToRgb(tintColor);
    gl.uniform3f(u.u_lightColor ?? 0, light[0], light[1], light[2]);
    gl.uniform3f(u.u_darkColor ?? 0, dark[0], dark[1], dark[2]);
    gl.uniform1f(u.u_sharp ?? 0, patternSharpness);
    gl.uniform1f(u.u_wave ?? 0, waveAmplitude);
    gl.uniform1f(u.u_noise ?? 0, noiseScale);
    gl.uniform1f(u.u_chroma ?? 0, chromaticSpread);
    gl.uniform1f(u.u_distort ?? 0, distortion);
    gl.uniform1f(u.u_contour ?? 0, contour);
    gl.uniform3f(u.u_tint ?? 0, tint[0], tint[1], tint[2]);
  }, [
    ready,
    seed,
    scale,
  ...
  useEffect(() => {
    ...
    const resize = () => {
      const rect = canvas.getBoundingClientRect();
      if (rect.width > 0 && rect.height > 0 && u.u_ratio) {
    ...
    const render = (time: number) => {
      ...
      gl.uniform1f(u.u_time ?? 0, animTimeRef.current);
