...
const actions = [
  ...
  { id: "CARTIS-117", state: "Done", comment: "Shipped Aug 10 (cc4e64a, deploy 78a6b3e7): settings modal content scroll — DialogContent is a flex column with overflow-hidden; settings-panel root is flex-1 min-h-0, pane min-h-0 flex-1 overflow-y-auto; verified live: dialog height 425px (viewport-capped), pane scrollHeight 1261 vs clientHeight 377, scrolls to 884px." },
  { id: "CARTIS-118", state: "Done", comment: "Shipped Aug 10 (eaa6f77, deploy 78a6b3e7): Lenis scroll re-sync on route change — SmoothScrollProvider tracks usePathname; on change: lenis.scrollTo(0, immediate) + lenis.resize() so the smooth-scroll engine's internal state matches the new page; verified live: wheel scrolls (window.scrollY=810) on landing after SPA nav from app." },
  { id: "CARTIS-119", state: "Done", comment: "Shipped Aug 10 (56379e3, deploy 78a6b3e7): switch account reworked — item visible for all users (personal -> 'Switch to Business', business -> 'Switch to Personal finance'); clicking calls updateUserType mutation then window.location.reload() (was gated to business users + router.push same-URL no-op); verified live: personal user sees item, clicks, reloads into business state with flipped menu item." },
  { id: "CARTIS-120", state: "Done", comment: "Shipped Aug 10 (56379e3, 78a6b3e7): model switcher full pipeline verified — UI dropdown (Llama 4 Scout default, Llama 3.3 70B, Llama 3.1 8B) persists via setAiModel mutation to users.ai_model, backend /chat/stream reads fetch_model per request, gateway forwards without override; verified live: dropdown opens, items listed, click selects Llama 3.3, DB shows aiModel = @cf/meta/llama-3.3-70b-instruct." },
];
