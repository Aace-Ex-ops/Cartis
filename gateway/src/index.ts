...
app.post('/api/internal/yodlee-sweep', async (c) => {
  ...
  for (const login of logins) {
    ...
    try {
      ...
      out.accounts = ((acc.json.account ?? []) as unknown[]).length
      out.accountTypes = JSON.stringify(((acc.json.account ?? []) as { accountType?: string }[]).slice(0, 5).map((a) => a.accountType ?? ''))
...
// Polar product id → plan tier (production org "saptarshi").
const POLAR_PRODUCTS: Record<string, string> = {
  '57780d49-cca0-43eb-91fd-6a538e261f67': 'pro',
  'c70e8ce4-7151-4705-adbf-7c828a1cb7fe': 'max',
