...
- **Aug 12 — Seller twin captures (income/expense/inventory) + live-refresh pages**: `chat.rs` adds `SELLER_PROMPT` + `validate_income`/`validate_expense`/`validate_inventory`, extracted in parallel inside both `extract_captures` and `extract_captures_groq` (still in `tokio::join!`), packed by `pack_captures` alongside goal/holding/profile/budget/purchase. Sell-side prompt: income (category ∈ Product sales|Online orders|Wholesale|Other), expense (entry_type ∈ expense|cogs|salary|rent|other + category ∈ Materials|Payroll|Rent|Logistics|Utilities|Other), inventory (name/stock/unit_cost; SKU synthesized client-side). Validators hard-allowlist enum values, coerce to allowlisted defaults, drop ≤0 amounts, `None` on garbage — same trust pattern as the consumer five.
- **UI (twin-chat.tsx)**: 3 new capture card types (Record income / Record expense / Add to inventory) with `saveIncome`/`saveExpense`/`saveInventory` → `addFinanceEntry` / `addInventoryItem` mutations, each dispatching `DATA_CHANGED_EVENT` on success. **Live refresh**: income/expenses/inventory/pnl/tax/cashflow pages now use `useLiveData(load, [load])` (replaces loaded-ref + raw useEffect); coach page refactored into `load` callback (`useLiveData` wraps it) so a twin save re-fetches every seller page in place. Cleanup: `useEffect`/`useRef` imports dropped where unused.
- **Deploy**: backend via SSM (SSH port blocked) — gzip→base64 `chat.rs` (16KB, well under the 97KB send-command cap) → `install` to `/home/ubuntu/api/src/chat.rs` → `su ubuntu -c 'cargo build --release'` (2m19s) → stop → cp binary → start. `sslip.io/api/health` answers (401 secret gate) = new binary live. UI still ships via user's wrangler (out/ deploy).
- **GOTCHA — TWO SOURCE TREES (bit us again)**: the box has `/home/ubuntu/api` (LIVE: systemd WorkingDirectory+ExecStart, what actually builds) AND `/home/ubuntu/api-src` (orphaned). Uploading ONLY `chat.rs` into the live tree and rebuilding produced a binary WITHOUT `addPurchase` — `graphql.rs` in `/home/ubuntu/api/src` was stale (0× add_purchase; api-src was current 1×). Symptom: UI "Couldn't add purchase — try again." + curl on box → `Unknown field "addPurchase" on type "MutationRoot"`, while DB showed purchases saved fine from the pre-deploy (api-src) binary. FIX (always): ship the WHOLE tree — `tar czf /tmp/api-src.tar.gz src/*.rs Cargo.toml Cargo.lock` (72KB base64, fits cap) → extract into `/home/ubuntu/api` → build → stop → cp → start. Verify both md5s match local before building. Verified live: `{"data":{"addPurchase":true}}` from localhost curl with secrets headers.
- **Aug 12 (2nd deploy) — direction-aware twin captures**: `pack_captures` now drops `purchase` whenever a seller capture fires (income/expense/inventory) — "sold 3 dresses for 500 each" = income card only, no bogus purchase card. `SELLER_PROMPT` rewording: SELL → income; BUY goods/stock/raw material for the shop → expense (cogs/Materials); personal buys stay on consumer Purchase Tracker. Deployed full-tree tar.gz (md5s verified: chat `83ecf548`, graphql `efdc24b1`), build 2m12s, health 401 = live.
- **Aug 15 — removed `signup`/`login` GraphQL mutations** (schema no longer exposes them; UI sign-in is Google-only via `/auth/start?intent=`). Deleted the password-hash fields from `MutationRoot` + the `AuthUser` type + argon2 imports in graphql.rs (admin.rs password auth + main.rs setup untouched). Deployed via SSH (port 2222, `scp -P 2222`), NOT ssm: scp ALL `src/*.rs` + Cargo.toml/lock. GOTCHA: `Cargo.toml/lock` in `/home/ubuntu/api` are owned by mac uid **501/staff** (unwritable by ubuntu) → scp to `/tmp` then `sudo install -o ubuntu -g ubuntu`. Old `graphql.rs.bak` in remote src/ removed. Build 2m16s → stop → cp binary → start; health 401 = live.
- **Aug 15 — UI overhaul from `Cartis-Copy` (sibling repo at `3d98e42`, = Cartis + PR #9)**: swapped in the redesigned UI (new sidebar/header/dashboard-shell, consumer components `accounts-tree`/`net-worth-chart`/`bills-income-card`/`income-spending-summary`/`recent-transactions-card`/`cursor-gradient`, rebuilt dashboard pages, new landing, `/pricing`). api between the two trees was byte-identical; only net delta was 30 UI files (+2214/−1225). Method: rsync from `Cartis-Copy/ui/{src,public}` (package.json unchanged). GOTCHAS: rsync excludes need the path WITHOUT the `src/` prefix or they silently clobber (kept 3 local files via backup restore: `(app)/layout.tsx`, `subscription-panel.tsx`, `twin-chat.tsx`). Re-applied local features onto new shell: `effectivePlan` threading (`dashboard-shell`→`Sidebar`→`UserMenu`→panels), search box in new `header-bar` → `/dashboard/search` (untracked page kept), real Google sign-in kept (PR#9 shipped a mock localStorage login — rewire = restore local `signin-form.tsx`; `lib/auth.ts` keep new version, `/auth/me` primary + dev-only demo fallback). Fixed build: `purchases/page.tsx` missing `Verdict` type + `SkeletonHeading/Card/Row` import. Build `next build` OK; `wrangler deploy` from `gateway/` (assets `../ui/out`), live at cartis.dpdns.org (first deploy attempt hit a transient fetch error; retry succeeded). **Static/demo (no backend fetch)**: home `/dashboard`, `/dashboard/analysis`, `personal-dashboard.tsx` (PR#9 dummy data) — wiring to live gql is follow-up work. Backup of pre-swap files: `$TMPDIR/opencode/cartis-ui-backup`.
